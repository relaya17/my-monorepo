...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    langBtnRef.current?.setAttribute('aria-expanded', String(langOpen));
  }, [langOpen]);

  useEffect(() => {
    toggleBtnRef.current?.setAttribute('aria-expanded', String(isOpen));
  }, [isOpen]);
  ...
  return (
    <nav className={`navbar navbar-expand-lg navbar-dark navbar-glass ${IS_LANDING(pathname) ? 'navbar-transparent' : ''}`}>
      <div className="container">
        <Link className="navbar-brand text-white" to="/" onClick={handleCloseNavbar}>
          <span className="brand-stack">
            <span className="brand-name text-white fw-bold fs-3">
              VANTERA
            </span>
