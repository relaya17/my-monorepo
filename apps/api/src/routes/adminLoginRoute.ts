...
// POST /api/admin/register - רישום אדמין חדש (חסום בסביבת production – דורש super-admin)
router.post('/register', loginRateLimiter, async (req: Request, res: Response) => {
    // Block open registration in production
...
router.post('/login', loginRateLimiter, async (req: Request, res: Response) => {
    ...
    if (process.env.NODE_ENV === 'development') console.log('Admin login attempt:', { username, hasPassword: Boolean(password) });

    if (!username || !password) {
        if (process.env.NODE_ENV === 'development') console.log('Missing username or password');
        return res.status(400).json({ message: 'יש להזין שם משתמש וסיסמה' });
    }
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') console.log('Admin found:', !!admin);

        if (!admin) {
            if (process.env.NODE_ENV === 'development') console.log('Admin not found');
            return res.status(401).json({ message: 'שם משתמש או סיסמה שגויים' });
        }

        const pwd = admin.password;
        const isMatch = typeof pwd === 'string' && (await bcrypt.compare(password, pwd));
        if (process.env.NODE_ENV === 'development') console.log('Password match:', isMatch);

        if (!isMatch) {
            if (process.env.NODE_ENV === 'development') console.log('Password does not match');
            return res.status(401).json({ message: 'שם משתמש או סיסמה שגויים' });
        }
